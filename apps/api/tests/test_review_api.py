...
from papertrace_core.storage import create_job, init_db, reset_storage_state, update_job_status  # noqa: E402
...
def test_review_large_file_fallback_serves_prebuilt_html(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert (
            file_body["fallback_html_path"]
            == f"/api/v1/analyses/{job.id}/review/files/{file_id}/rendered"
        )
